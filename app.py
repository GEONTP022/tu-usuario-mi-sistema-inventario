...
# 2. CARGA DE MERCANCÍA (OBLIGATORIA)
elif opcion == "Carga":
...
# 3. ESTADÍSTICAS
    ...
    with col1:
        if h_data:
            ...
            fig = px.bar(salidas.nlargest(10, 'cantidad'), x='producto_nombre', y='cantidad', title="Top 10 Más Vendidos", template="plotly_dark")
            st.plotly_chart(fig, use_container_width=True)
    with col2:
        if p_data:
            df_p = pd.DataFrame(p_data)
            fig_p = px.pie(df_p, names='categoria', values='stock', hole=0.4, title="Distribución por Categoría", template="plotly_dark")
            st.plotly_chart(fig_p, use_container_width=True)

# 4. HISTORIAL
elif opcion == "Log":
    st.header("📜 Historial de Salidas (Quién y Qué)")
    logs = supabase.table("historial").select("*").order("fecha", desc=True).execute().data
    if logs:
        st.table(pd.DataFrame(logs)[['fecha', 'producto_nombre', 'cantidad', 'usuario']])

# 5. USUARIOS Y PROVEEDORES (Se mantienen igual)
elif opcion == "Users":
