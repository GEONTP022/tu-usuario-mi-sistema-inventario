...
# --- CSS LIMPIO (SIN CÓDIGO QUE ROMPA EL LAYOUT) ---
st.markdown("""
    <style>
    ...
    /* Imágenes de productos centradas */
    ...
    with st.form("form_nuevo_prod"):
        ...
        if st.form_submit_button("GUARDAR"):
            if not n or c == "Seleccionar" or p_gen <= 0:
                ...
                # VALIDACIÓN: Nombre + Marca + Categoría + Código idénticos
                existe = supabase.table("productos").select("id")\
                    .eq("nombre", n).eq("marca", m).eq("categoria", c).eq("codigo_bateria", cb).execute()

                if existe.data:
                    st.error("⚠️ Ya existe este producto exacto.")
                else:
...
if opcion == "Stock":
    ...
    if items:
        ...
        # --- GRID SYSTEM (Nativo de Streamlit, sin CSS raro) ---
        # Esto evita que se superpongan los elementos
        N_COLS = 4
            ...
            for i, p in enumerate(row):
                with cols[i]:
                    with st.container(border=True):
                        ...
                        c1.metric("Stock", p['stock'])
                        c2.metric("Gral", f"S/{p['precio_venta']}")
                        c3.metric("Punto", f"S/{p.get('precio_punto', 0)}")
        ...
        with st.container(border=True):
            ...
            with st.form("edit_form"):
                ...
                with c1:
                    ...
                    new_cod = st.text_input("Código", value=prod.get('codigo_bateria', ''))
                with c2:
                    new_p_gen = st.number_input("Precio Gral", value=float(prod['precio_venta']))
                    new_p_punto = st.number_input("Precio Punto", value=float(prod.get('precio_punto', 0)))
    ...
    if provs:
        for p in provs:
            with st.container(border=True):
                ...
                st.link_button("WhatsApp", f"https://wa.me/{p['whatsapp']}")
